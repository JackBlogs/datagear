...
// 角色管理列表，按原 role_table.ftl 复刻：搜索、增删改查、多选、排序、分页。
...
function formatEnabled(data: Role): string {
...
function onView() {
  const id = selected.value[0]?.id
  if (!id) return
  router.push({ path: `/role/${id}/view`, query: { mode: 'view' } })
}
  ...
  <div class="page page-manager page-table h-full flex flex-column overflow-auto p-1">
    ...
    <div class="page-content flex-grow-1 overflow-auto">
      ...
      >
        ...
        <Column field="id" :header="t('id')" sortable class="col-id" />
        <Column field="name" :header="t('name')" sortable class="col-name" />
        <Column field="enabled" :header="t('enable')" sortable class="col-boolean">
