...
// 看板管理列表，按原 dashboard_table.ftl 复刻：搜索、增删改查、分享、导入、多选、排序、分页。
...
const selected = ref<DashboardListItem[]>([])
const keyword = ref('')
...
function onView() {
  const id = selected.value[0]?.id
  if (!id) return
  router.push({ path: `/dashboard/${id}/view`, query: { mode: 'view' } })
}

function onShare() {
  const id = selected.value[0]?.id
  if (!id) return
  router.push(`/authorization/Dashboard/${id}`)
}
  ...
  <div class="page page-manager page-table h-full flex flex-column overflow-auto p-1">
    <div class="page-header grid grid-nogutter align-items-center p-1 flex-grow-0">
      ...
      <div class="operations col-12 flex gap-1 flex-wrap md:justify-content-end md:col-8">
        ...
        <Button :label="t('import')" size="small" @click="onImport" />
        ...
        <Button :label="t('share')" size="small" :disabled="selected.length !== 1" @click="onShare" />
    ...
    <div class="page-content flex-grow-1 overflow-auto">
      ...
      >
        ...
        <Column field="apiVersion" :header="t('version')" sortable class="col-version" />
