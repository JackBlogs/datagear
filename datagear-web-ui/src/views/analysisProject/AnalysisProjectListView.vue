...
// 分析项目管理列表，按原 analysisProject_table.ftl 复刻：搜索、增删改查、多选、排序、分页。
...
const selected = ref<AnalysisProject[]>([])
const keyword = ref('')
...
function onEdit() {
  ...
  if (!id) return
  router.push(`/analysisProject/${id}/edit`)
}

function onView() {
  const id = selected.value[0]?.id
  if (!id) return
  router.push({ path: `/analysisProject/${id}/view`, query: { mode: 'view' } })
}
  ...
  <div class="page page-manager page-table h-full flex flex-column overflow-auto p-1">
    ...
    <div class="page-content flex-grow-1 overflow-auto">
      ...
      >
        ...
        <Column field="description" :header="t('description')" sortable class="col-desc" />
