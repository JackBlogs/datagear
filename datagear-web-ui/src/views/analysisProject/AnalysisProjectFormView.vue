...
async function load() {
  ...
  loading.value = true
  try {
    form.value = await moduleGet('analysisProject', id)
  } catch (e) {
...
async function save() {
  ...
  saving.value = true
  try {
    await moduleSave('analysisProject', form.value)
    success('保存成功')
...
<template>
  <div class="p-4">
    <div class="flex align-items-center gap-2 mb-3">
      <h3 class="flex-1">{{ isEdit ? '编辑分析项目' : '新建分析项目' }}</h3>
      <Button label="返回" text @click="router.push('/analysisProject')" />
    </div>
    <div v-if="loading" class="text-color-secondary">加载中…</div>
    <div v-else class="form flex flex-column gap-3">
      <div class="flex align-items-center gap-2">
        <label class="label">名称</label>
        <input v-model="form.name" class="input flex-1" placeholder="项目名称" />
      </div>
      <div class="flex align-items-center gap-2">
