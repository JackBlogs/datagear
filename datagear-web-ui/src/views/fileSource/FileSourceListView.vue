...
import { fileSourcePagingQueryData, deleteFileSources, type FileSource } from '@/api/fileSource'
import { useOperationMessage } from '@/composables/useOperationMessage'
import type { Order } from '@/types'

// 文件源管理列表，按原 fileSource_table.ftl 复刻：搜索、增删改查、多选、排序、分页。
...
const keyword = ref('')
const sortMeta = ref<any[]>([{ field: 'name', order: 1 }])
...
function onEdit() {
  ...
  if (!id) return
  router.push(`/fileSource/${id}/edit`)
}

function onView() {
  const id = selected.value[0]?.id
  if (!id) return
  router.push({ path: `/fileSource/${id}/view`, query: { mode: 'view' } })
}
  ...
  <div class="page page-manager page-table h-full flex flex-column overflow-auto p-1">
    <div class="page-header grid grid-nogutter align-items-center p-1 flex-grow-0">
      <div class="col-12 md:col-4">
        <form class="flex gap-1" @submit.prevent="search">
          <InputText v-model="keyword" :placeholder="t('searchByNameOrDesc')" class="flex-1" />
    ...
    <div class="page-content flex-grow-1 overflow-auto">
      ...
      >
        ...
        <Column field="description" :header="t('description')" sortable class="col-desc col-last" />
      </DataTable>
    </div>
  </div>
</template>
